...
CREATE TABLE timebookapp.users (
    id                SERIAL              PRIMARY KEY,
    version           BIGINT     NOT NULL DEFAULT 1,
    nickname          VARCHAR(1) NOT NULL PRIMARY KEY CHECK(char_length(nickname) BETWEEN 1 AND 25),  
    password          VARCHAR    NOT NULL PRIMARY KEY ,  
    registrated_at    TIMESTAMP  NOT NULL,
    is_admin          BOOLEAN, 
    avatar            VARCHAR,
)

CREATE TABLE todobookapp.slot_participants (
    id             SERIAL           PRIMARY KEY,
    version        BIGINT  NOT NULL DEFAULT 1,
